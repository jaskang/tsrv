...
export interface Options
  extends Pick<SFCTemplateCompileOptions, 'compiler' | 'compilerOptions' | 'transformAssetUrls'>,
      ...
      if (query.vue) {
        ...
        if (query.type === 'template') {
          ...
          const result = await compileStyleAsync({
            ...
            preprocessLang: options.preprocessStyles ? (block.lang as any) : undefined,
            preprocessCustomRequire: options.preprocessCustomRequire
          })
...
function transformVueSFC(
  ...
  { rootContext, isProduction, isServer }: { rootContext: string; isProduction: boolean; isServer: boolean },
  options: Options
) {
...
function getStyleCode(descriptor: SFCDescriptor, resourcePath: string, id: string, preprocessStyles?: boolean) {
  let stylesCode = ``
